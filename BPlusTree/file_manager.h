...
// using T = double;
// constexpr size_t table_size = 1019;
// constexpr size_t cache_size = 100;
// constexpr size_t bias       = 0;
...
class file_manager {
  private:
    ...
    [[no_unique_address]]i_func reader; /* Read  only. */
    [[no_unique_address]]o_func writer; /* Write only. */
    ...
    void insert_map(iterator &iter,file_state index) {
        ...
        iter = map.insert_after(index,cache,iter,map.size() == cache_size);
    }

    /* Locate the position for reading. */
    void locate_in (size_t index) 
    { dat_file.seekg(index * sizeof(T) + bias); }
    /* Locate the position for writing. */
    void locate_out(size_t index) 
  ...
  public:
    ...
    visitor get_object(size_t index) {
        ...
        insert_map(iter,{index}); /* Insert to map from cache. */
